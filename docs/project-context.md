...
- Providers can be shown one of three visual style options for their page, differing only in look and feel (accent color, neutral family, typeface, corner radius), as part of a sales conversation about whether they want a white-label page.
...
## Inline booking widget, current direction (2026-08-24)

All work now lands in one continuously-revised comp: **[Inline Book Widget](https://claude.ai/code/artifact/c2fe7103-10a3-4fb2-a2f7-dedf5bb76e46)**.

- **Placement:** one CSS Grid with named areas relocates the same widget markup between breakpoints — under the header on mobile, a sticky sidebar column on desktop — rather than duplicating it per layout.
- **Progressive, 5 steps:** Service → Date & Time + Pets → Add-ons → Your Info → Review. Each completed step collapses to a one-line recap with "Change," and Add-ons was deliberately placed as its own step near the end rather than folded into Service selection. A pet must be added before Continue unlocks past the Date & Time + Pets step.
...
This is now the current direction for the button-swap exploration.
